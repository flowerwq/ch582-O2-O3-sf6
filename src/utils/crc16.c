...
void crc16_init(crc16_ctx_t *ctx){
	...
	memset(ctx, 0, sizeof(crc16_ctx_t));
	ctx->crcdata.unit.high = 0xff;
	ctx->crcdata.unit.low = 0xff;
}
...
void crc16_update(crc16_ctx_t *ctx, uint8_t *buf, uint16_t len){
	...
	for (i = 0; i < len; i++)
	{  
		index = ctx->crcdata.unit.low ^ buf[i];
        ctx->crcdata.unit.low = ctx->crcdata.unit.high ^ auchCRCHi[index];
        ctx->crcdata.unit.high = auchCRCLo[index]; 
	}
	ctx->len += len;
}
